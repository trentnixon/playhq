...
const useGroupedData = (switchValue) => {
  ...
  const { data: organizationDetails, loading } = useGetOrganizationDetails(AccType, accountId);

  const currentDate = new Date();
  const filteredComps = useMemo(() => {
    return (organizationDetails?.data?.attributes?.competitions?.data || []).filter(
  ...
  const groupedComps = useMemo(() => {
    if (AccType === "Club") {
      return switchValue
        ? categorizeTeamsByAgeGroup(organizationDetails?.data?.attributes?.teams?.data)
        : categorizeTeamsBySeniorJunior(organizationDetails?.data?.attributes?.teams?.data);
    } else {
      return switchValue
