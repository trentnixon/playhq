...
export const defineLogo = (accountData, userLogoUrl, defaultLogo) => {
    ...
    return accountData.attributes.Logo?.data?.attributes?.url || accountData.attributes?.ParentLogo || userLogoUrl || defaultLogo;
  };
    ...
    getDominantColors(logoUrl)
      .then((colors) => {
        if (colors && colors.length >= 2) {
          updatedData.VIDEOMETA.Video.Theme.primary = userColors[0] ? userColors[0] : colors[0];
          updatedData.VIDEOMETA.Video.Theme.secondary = userColors[1] ? userColors[1] : colors[1];
        }
        resolve(updatedData);
      })
      .catch((error) => {
...
export const updateTop5RunScorers = (data, logoUrl, accountName) => {
  ...
  export const updateDataWithClubInfo = (AccountData, initialData, defaultLogo) => {
