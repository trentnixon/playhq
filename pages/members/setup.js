...
const SETUP = () => {
  ...
  // Initialize ReRender and account data - only run once on mount
  useEffect(() => {
    // Only call ReRender if we don't have account data yet and haven't initialized
    if (!account && !hasInitializedRef.current) {
  ...
  const Content = UserAccount ? (
    <SetupStages setReview={setReview} />
  ) : (
    <InitialSetupScreen />
  );
