...
export const FromMiddle = (START = 0, EASE = 'fast') => {
  return `inset(0 ${SpringToFrom(START, 50, 0, EASE)}% 0 ${SpringToFrom(
    START,
    50,
    0,
    EASE
...
export const EraseFromMiddle = (START = 0, EASE = 'fast') => {
  return `inset(0 ${SpringToFrom(START, 0, 50, EASE)}% 0 ${SpringToFrom(
    START,
    0,
    50,
    EASE
...
export const EraseToMiddleFromTop = (START = 0, EASE = 'fast') => {
  return `inset(${SpringToFrom(START, 0, 50, EASE)}% 0 ${SpringToFrom(
    START,
    0,
    50,
    EASE
...
export const FromTopToBottom = (START = 0, EASE = 'fast') => {
  return `inset( 0 -4px ${SpringToFrom(START, 100, 0, EASE)}% 0)`;
};
...
export const SquareWipe = (START = 0, EASE = 'fast') => {
  return `inset(${SpringToFrom(START, 50, 0, EASE)}% ${SpringToFrom(
    START,
    50,
    0,
    EASE
  )}% ${SpringToFrom(START, 50, 0, EASE)}% ${SpringToFrom(
    START,
    50,
    0,
    EASE
