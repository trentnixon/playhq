...
import { StepHeaderandDescription } from "../../pages/members/SetupSteps/StepHeaderandDescription";
...
export const FixturaAIsettings = ({ user, setHasUpdated, showDayOfWeek=true }) => {
  console.log(user);
  return (
    <>
    {
      showDayOfWeek?(<><LabelMe label="Select a Day of the Week" />
      <ShadowWrapper>
    ...
    <LabelMe label="Select a publication style for your Write ups" />
      <ShadowWrapper size={"lg"}> 
        <SelectFixturaSetting
          ...
          SelectedBaseValueObject={user.attributes?.ai_publication?.data?.attributes}
      ...
      <SelectFixturaSetting
        ...
        COLLECTIONID={user.id}
        SelectedBaseValueObject={user.attributes?.ai_writting_style?.data?.attributes}
